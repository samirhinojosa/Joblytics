...
def wttj_scrapper(
    ...
    time_posted: TimePosted = typer.Argument(
        TimePosted.DAY,
        help="WTTJ publication date: all, day (24h), week (7d), month (30d).",
    ),
    work_modality: WorkModality = typer.Argument(
        WorkModality.ALL,
        help="WTTJ work modality: all, onsite, hybrid, remote.",
    ),
    show_table: bool = typer.Option(
        False,
        "--show-table/--no-show-table",
    ...
    """
    Extracts job postings from Welcome to the Jungle and displays a summary
    ...
    try:
        color = typer.colors.GREEN
        typer.secho(
            f"✨ [WTTJ] scrape started (title={title}, "
            f"location={location}, time_posted={time_posted.value}, "
            f"work_modality={work_modality.value}, dry_run={dry_run}).",
            fg=color,
        )
        ...
        color = typer.colors.GREEN if report.produced > 0 else typer.colors.YELLOW
        typer.secho("🟢 [WTTJ] Finalized.", fg=color)
        ...
        logger.exception("Error running wttj-scrape")
        typer.secho(f"🔴 Error running wttj-scrape: {e}", err=True, fg=typer.colors.RED)
        raise typer.Exit(code=1)
